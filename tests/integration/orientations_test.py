...
def test_normalize(vasp_orientations):
    ...
    np.testing.assert_allclose(first, [0.300804, 0.433833, -0.849297], atol=1e-06)
...
def test_conventional(vasp_orientations):
    ...
    first = ret.vectors[0, 0]
    np.testing.assert_allclose(first, [-0.712261, 1.378551, -0.213198], atol=1e-06)
...
def test_symmetrize(vasp_orientations):
    ...
    first = ret.vectors[0, 0]
    np.testing.assert_allclose(first, [1.330221, -0.679495, -0.471138], atol=1e-06)
...
def test_normalize_symmetrize(vasp_orientations):
    ...
    first = ret.vectors[0, 0]
    np.testing.assert_allclose(first, [0.849297, -0.433833, -0.300804], atol=1e-06)
...
def test_fractional_coordinates(vasp_orientations):
    direction = vasp_orientations._fractional_directions(vasp_orientations._distances)
...
def test_matching_matrix(vasp_orientations):
    ...
    matching_matrix = vasp_orientations._matching_matrix(distance, frac_coord_cent)
