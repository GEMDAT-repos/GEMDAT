...
class TestTransitions:  # type: ignore
    ...
    def test_site_radius(self, vasp_traj, structure):
        site_radius = SiteRadius.from_vibration_amplitude(
            trajectory=vasp_traj,
            sites=structure,
            vibration_amplitude=0.5204,
        )
        assert isclose(site_radius.radius[''], 0.9284961123176741)
    ...
    def test_all_transitions_matrix(self, vasp_transitions):
        ...
        assert_allclose(
            np.argwhere(matrix)[::50], np.array([[0, 95], [49, 95], [95, 5], [95, 55]])
        )

    def test_transitions_parts(self, vasp_transitions):
        n_sites = vasp_transitions.n_sites
        tp = np.stack([part.matrix() for part in vasp_transitions.split(n_parts=10)])

        assert tp.shape == (self.n_parts, n_sites, n_sites)
        assert tp.sum() == 2100
        assert_allclose(
            np.argwhere(tp)[::500], np.array([[0, 0, 95], [4, 23, 95], [8, 28, 95]])
        )
    ...
    def test_occupancy_parts(self, vasp_transitions):
        ...
        assert values1 == [0.11066666666666666, 0.708, 0.408, 0.62, 0.42533333333333334]
