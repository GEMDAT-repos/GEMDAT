...
class Trajectory(PymatgenTrajectory):
    ...
    def extend(self, other: Trajectory) -> Trajectory:
        """Extend trajectory by appending positions from the other trajectory.

        Parameters
        ----------
        other : Trajectory
            Append positions from this trajectory.
            Metadata between both trajectories must match.
        ...
        ------
        ValueError
            If any metadata does not match.
        """
        if self.time_step != other.time_step:
            raise ValueError('Time step is different.')
        if self.species != other.species:
            raise ValueError('Species are different.')
        if self.get_lattice() != other.get_lattice():
            raise ValueError('Lattices are different.')
        if self.metadata != other.metadata:
            raise ValueError('Metadata are different.')
