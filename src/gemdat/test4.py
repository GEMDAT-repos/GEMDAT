...
for i in range(len(sym_ops_Oh_firstcolumn)):
    for j in range(len(sym_ops_Oh_firstrow)):
        sym_ops_Oh[i, j, :, :] = np.matmul(sym_ops_Oh_firstcolumn[i, :, :],
...
for m in range(n_ts):
    for l in range(n_bonds):
        for k in range(n_symops):
            direction_cart_symOh[m, l * n_symops + k, :] = np.matmul(
                sym_ops_Oh[:, :, k], direction_cart[m, l, :])
...
#%%
direction_spherical_deg = gr.cartesian_to_spherical(direction_cart_symOh,
                                                    degrees=True)
