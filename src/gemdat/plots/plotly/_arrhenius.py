...
def arrhenius(*, fit: ArrheniusFit, show_std: bool = True) -> go.Figure:
    ...
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=x, y=y, mode="markers", name="data", error_y=error_y))

    # Fit line
    t_line = np.linspace(float(T.min()), float(T.max()), 200)
    x_line = 1000.0 / t_line
    ln_line = fit.intercept + fit.slope * (1.0 / t_line)
    fig.add_trace(go.Scatter(x=x_line, y=ln_line, mode="lines", name="fit"))
    ...
    if show_std and getattr(fit, "cov", None) is not None:
        ...
        fig.add_trace(go.Scatter(x=x_line, y=upper, mode="lines", line=dict(width=0), showlegend=False))
        fig.add_trace(
            go.Scatter(
                x=x_line,
                y=lower,
