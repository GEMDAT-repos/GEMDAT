...
def plot_jump_paths(sites, lat, show_names):
    ...
    for i in range(3):
        ...
        for j in range(3):
            if i != j:
                ax.plot([lat[i, 0], lat[i, 0] + lat[j, 0]],
                        [lat[i, 1], lat[i, 1] + lat[j, 1]],
                        [lat[i, 2], lat[i, 2] + lat[j, 2]],
                        color='black')
                for k in range(3):
                    if k != i and k != j:
                        ax.plot([
                            lat[i, 0] + lat[j, 0],
                            lat[i, 0] + lat[j, 0] + lat[k, 0]
                        ], [
                            lat[i, 1] + lat[j, 1],
                            lat[i, 1] + lat[j, 1] + lat[k, 1]
                        ], [
                            lat[i, 2] + lat[j, 2],
                            lat[i, 2] + lat[j, 2] + lat[k, 2]
                        ],
                                color='black')
    ...
    for i in range(nr_sites):
        ...
        if point_size > 1:
            ax.scatter3D(sites['cart_pos'][0, i],
                         sites['cart_pos'][1, i],
                         sites['cart_pos'][2, i],
                         s=point_size,
                         color='blue')
            if show_names:
                ax.text(sites['cart_pos'][0, i] + delta,
                        sites['cart_pos'][1, i] + delta,
                        sites['cart_pos'][2, i] + delta,
                        sites['site_names'][i])
    ...
    for i in range(nr_sites - 1):
        for j in range(i + 1, nr_sites):
            ...
            if sites['transitions'][i, j] + sites['transitions'][j, i] > 0:
                linewidth = 1 + np.log(sites['transitions'][i, j] +
                                       sites['transitions'][j, i])
            if linewidth > 0:
                color = 'red'
                for k in range(3):
                    vec_cart[k, :] = [
                        sites['cart_pos'][k, i], sites['cart_pos'][k, j]
                    ]
                    vec_frac[k, :] = [
                        sites['frac_pos'][k, i], sites['frac_pos'][k, j]
                    ]

                pbc = np.array([False, False, False])
                nr_pbc = 0
                for k in range(3):
                    if abs(sites['frac_pos'][k, i] -
                           sites['frac_pos'][k, j]) > 0.5:
                        pbc[k] = True
                        if nr_pbc == 0:
                            nr_pbc = 1
                ...
                if np.any(pbc):
                    for k in range(3):
                        if not pbc[k]:
                            vec1[k, :] = [
                                vec_frac[k, index_max], vec_frac[k, index_min]
                            ]
                            vec2[k, :] = [
                                vec_frac[k, index_max], vec_frac[k, index_min]
                            ]
                    ...
                    vec1a = frac_to_cart(
                        vec1[:, 0], lat)  # Convert to cartesian coordinates
                    vec1b = frac_to_cart(vec1[:, 1], lat)
                    vec2a = frac_to_cart(vec2[:, 0], lat)
                    vec2b = frac_to_cart(vec2[:, 1], lat)

                    ax.plot([vec1a[0], vec1b[0]], [vec1a[1], vec1b[1]],
                            [vec1a[2], vec1b[2]],
                            color=color,
                            linewidth=linewidth)
                    ax.plot([vec2a[0], vec2b[0]], [vec2a[1], vec2b[1]],
                            [vec2a[2], vec2b[2]],
                            color=color,
                            linewidth=linewidth)
                else:
